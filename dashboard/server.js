...
module.exports = async bot => {
    ...
    app.listen(config.dashboard.port, () => console.log(chalk.green(`Sunucu Aktifleşti Port ${config.dashboard.port}`)));
  ...
  app.get("/sponsor", (req, res) => {
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        if (!dde)
          return res.redirect(
            "/profil?error=true&message=Sistemimiz de Böyle Bir Link Bulamadım."
          );
        uptimedata.findOne({ code: req.params.code }, async function(err, docs) {
          if (docs.userID != req.user.id)
            return res.redirect(
              "/profil?error=true&message=Silmeye Çalıştığınız Link Size Ait Değil."
            );
          res.redirect(
            "/profil?success=true&message=Linkiniz Sistemimiz den Başarıyla Silindi."
          );
    ...
    app.get("/uptime/:code/edit", checkAuth, async (req, res) => {
        ...
        if (!dde)
          return res.redirect(
            "/error?error=true&message=Sistemimiz de Böyle Bir Site Bulunamadı."
          );
        res.render("edit", {
          bot,
          user: req.user,
    ...
    app.post("/uptime/:code/edit", checkAuth, async (req, res) => {
      ...
      function(err, docs) {})
      res.redirect(
        "/uptime/"+dde.code+"/edit?success=true&message=Başarılı Bir Şekilde Bilgiler Güncellendi.."
      );
    })
    ...
    app.post("/uptime", checkAuth, async (req, res) => {
      ...
      if (!rBody["link"]) {
        res.redirect("?error=true&message=Bir Link Girmelisiniz.");
      } else {
        if (!rBody["link"].match("https"))
          return res.redirect("?error=true&message=Geçerli Bir Link Girmelisiniz.");
        ...
        if (dd.length > 5)
          res.redirect("?error=true&message=Üzgünüm 5 Proje Limitinizi Doldurmuşsunuz.");

        if (dde)
          return res.redirect(
            "?error=true&message=Bu Link Zaten Sistemimiz de Bulunuyor Eğer Senin Olmadığını Düşünüyorsan Glitch Proje İsmini Değiştirmenizi Tavsiye Ederiz."
          );
        ...
        res.redirect(
          "?success=true&message=Linkiniz Başarıyla Sistemimize Eklendi."
        );
      }
    })
